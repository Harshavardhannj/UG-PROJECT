...
if(isset($_POST["getProduct"])){
	...
	$product_query = "SELECT * FROM foods LIMIT $start,$limit";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$pro_id    = $row['food_id'];
			$pro_title = $row['food_title'];
			$pro_price = $row['food_price'];
			$pro_image = $row['food_image'];
...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT a.food_id,a.food_title,a.food_price,a.food_image,b.id,b.qty FROM foods a,cart b WHERE a.food_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	}else{
		//When user is not logged in this query will execute
		$sql = "SELECT a.food_id,a.food_title,a.food_price,a.food_image,b.id,b.qty FROM foods a,cart b WHERE a.food_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				$product_title = $row["food_title"];
				$product_price = $row["food_price"];
				$product_image = $row["food_image"];
				$cart_item_id = $row["id"];
				$qty = $row["qty"];
    ...
    if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			<form method="post" action="login_form.php">
						...
						<tr>
							<td data-th="Product" >
								<div class="row">
									...
									<div class="col-sm-4 "><img src="food_images/'.$product_image.'" style="height: 70px;width:75px;"/>
				...
				if (!isset($_SESSION["uid"])) {
					echo '
							<script>alert("First Login then Add Cart")</script>
						...
						<form action="checkout.php" method="post">
							...
							while($row=mysqli_fetch_array($query)){
								...
								echo  	
									...
									<input type="hidden" name="item_name_'.$x.'" value="'.$row["food_title"].'">
								  	 <input type="hidden" name="item_number_'.$x.'" value="'.$x.'">
								     <input type="hidden" name="amount_'.$x.'" value="'.$row["food_price"].'">
